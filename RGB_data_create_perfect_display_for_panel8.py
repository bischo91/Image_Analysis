...
pathstr = r"C:\CS\python_ruby\image_process\RGB\Panel\Test Images".replace("\\","/")

path = os.path.abspath(pathstr)
# Read all files in the folder
allfiles = [f for f in listdir(path) if isfile(join(path,f))]
allfiles = [f for f in allfiles if 'cropped' not in f]
...
for j in range(0, len(imgfiles)):
    ...
    if img_detect == False:
        print('Corner not detected for ' + filename)
    else:
                ...
                for ii in range(x_min, x_max):
                    for jj in range(y_min, y_max):
                        ...
                        if abs(jj-y_min) < 5 or abs(jj-y_max) <5:
                            img_resized[ii,jj] = [255, 0, 0]
                pixel_data.append([len(pixel_data)+1, np.average(pixel_value_list), np.std(pixel_value_list)])


        img_block = img_resized[127:135,541:548]
        img_perfect = np.zeros((2400, 1799, 3))

        for i in range(0, np.shape(img_perfect)[0]):
            for j in range(0, np.shape(img_perfect)[1]):
                n = int(i%8)
                m = int(j%7)
                img_perfect[i,j,:] = img_block[n, m,:]

        plt.figure()
        plt.axis('off')
        plt.imshow(img_perfect.astype(np.uint8))
        perfect_img_file = path+'/' + filename.replace('.jpg','').replace('.JPG', '') + '_perfect.jpg'
        # cv2.imwrite(perfect_img_file, cv2.cvtColor((img_perfect), cv2.COLOR_RGB2BGR))
        plt.savefig(perfect_img_file)
        new_img_file = path+'/' + filename.replace('.jpg','').replace('.JPG', '') + '_temp.jpg'
        ...
        sheet.cell(row=7, column=5).value = '100% - STDEV'
        sheet.cell(row=8, column=5).value = '1-(STDEV/AVG)'

        pixel_average = np.average([j[1] for j in pixel_data])
        pixel_std = np.std([j[1] for j in pixel_data])
        Lmin = min([j[1] for j in pixel_data])
        Lmax = max([j[1] for j in pixel_data])

        sheet.cell(row=2, column=6).value = pixel_average
        sheet.cell(row=3, column=6).value = pixel_std
        sheet.cell(row=4, column=6).value = Lmin
        sheet.cell(row=5, column=6).value = Lmax
        sheet.cell(row=7, column=6).value = 100 - pixel_std
        sheet.cell(row=8, column=6).value = (1 -(pixel_std/pixel_average))*100
        sheet.cell(row=1, column=8).value = filename.replace('.JPG','').replace('.jpg','')
        ...
        sheet.add_image(img_excel, 'H2')
        sheet_overall.title = 'Overall'
        sheet_overall.cell(row=1, column=1).value = 'Vg (V)'
        sheet_overall.cell(row=1, column=2).value = '1-(STDEV/AVG)'
        sheet_overall.cell(row=j+2, column=1).value = Vg
        sheet_overall.cell(row=j+2, column=2).value = (1 -(pixel_std/pixel_average))*100
plt.show()
