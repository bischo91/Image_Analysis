...
class PostImageProcess:
    ...
    def num_of_interaction_in_radius(radius, img_skel, i, j):
        img_cir_temp = cv2.circle(np.zeros(img_skel.shape), (j,i), radius, (255,0,0), 1)
...
for n in range(0, len(onlyfiles)):
    ...
    if onlyfiles[n].split('.')[1] in ['jpg','tif','png']:
        ...
        for i in range(2, img_2.shape[0]-2):
            for j in range(2, img_2.shape[1]-2):
                ...
                if connectivity>3:
                    ...
                    #     # img_cir = cv2.circle(img_cir, (j,i), 3, (255,0,0), 1)
                    #     counter_cir +=1
                    sum_of_interaction = 0
                    for k in range(1, 11):

                        sum_of_interaction += PostImageProcess.num_of_interaction_in_radius(k, img_skel, i, j)
                    if sum_of_interaction>40:
                        img_cir[i,j] = 1
                        counter_cir +=1
                    interaction_list.append(sum_of_interaction)
        ...
        print(np.mean(interaction_list))
        print(np.max(interaction_list))
        print(np.min(interaction_list))
        plt.figure()
        x= np.linspace(0,len(interaction_list), len(interaction_list))
        plt.plot(x, interaction_list)
        ...
        # print(cv2.imread(img_2))
        # lines = cv2.HoughLines(cv2.imread(img_2),1,np.pi/180, 200)
        # img_in = np.float32(img_2)
        # edges = cv2.Canny(np.uint8(img_2), 100, 200)
        # plt.imshow(edges)
        # arbitrary = 150
        # img_2 = img_skel
        # img_2 = img_2[100:150, 100:150]
        # minLineLength = 1000
        # lines = cv2.HoughLinesP(np.uint8(img_2),10,np.pi/180, 100)
        # plane = np.zeros(img_2.shape)
        ...
        #     #     # cv2.line(, pt1, pt2, (arbitrary, 234, arbitrary), 2, cv2.LINE_AA)
